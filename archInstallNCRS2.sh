...
cliInstall() {
    ...
    echo -e -n "What is your processor ${DARK_BLUE}I${NOCOLOR}ntel or ${DARK_RED}A${NOCOLOR}MD (${DARK_BLUE}i${NOCOLOR}/${DARK_RED}${NOCOLOR})? "
    old_stty_cfg=$(stty -g)
    stty raw -echo
...
ncursesManualInstall2() {
    ...
    dialog --title "$default_title 11/$steps" --infobox "Saving the keymap in /etc/vconsole.conf" 25 90 && clear
    if [ -e /etc/vconsole.conf ]; then
        echo "KEYMAP=$usrkymp" >> /etc/vconsole.conf
    else
        touch /etc/vconsole.conf
        echo "KEYMAP=$usrkymp" >> /etc/vconsole.conf
    fi
