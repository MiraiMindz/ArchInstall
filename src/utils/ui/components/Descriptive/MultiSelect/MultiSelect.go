...
func GetAnswer(sel any) []string {
	var sa []string
	for _, v := range sel.(model).answers {
		sa = append(sa, v)
	}
	return sa
}

func MultiSelect(style lipgloss.Style, width, height int, prompt string, options ...option) model {
	return model{
		prompt:  prompt,
		options: options,
		style:   style,
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case " ":
			_, ok := m.answers[m.cursor]
			if ok {
				delete(m.answers, m.cursor)
			} else {
				m.answers[m.cursor] = m.options[m.cursor].item
			}
		}
	}
	if !m.ready {
